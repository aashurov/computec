      ...
      <body>  
        <div class="container box">
          ...
          foreach($array as $value) {
            ...
            $float  = ceil(floatval($priceold));
            $vendor_code = $value["vendor_code"];
            if ($priceold >= 2100000)
              {
                $pricesum = $float + $float*0.05;
                $pricedol = $float/10230;
              }
              else if ($float <= 5115000)
              {
                $pricesum = $float + $float*0.04;
                $pricedol = $float/10230;
              }
              else if ($float >= 5115000)
              {
                $pricesum = $float + $float*0.03;
                $pricedol = $float/10230;
              }
              echo "Kodi " . $vendor_code . " - " . $float . " sum " . ceil($float/10230) . " $ " . ceil($pricesum) . " sum " . ceil($pricedol) . " $ "."<br/>";
            $image = $value["image"];
            $description = $value["description"];
            $name = $value["class"];
            $category_id = $value["category_id"];
              $priceneww = ceil($pricedol);
            $query1 = "INSERT INTO `oc_product` (`product_id`, `model`, `sku`, `upc`, `ean`, `jan`, `isbn`, `mpn`, `location`, `quantity`, `stock_status_id`, `image`, `manufacturer_id`, `shipping`, `price`, `points`, `tax_class_id`, `date_available`, `weight`, `weight_class_id`, `length`, `width`, `height`, `length_class_id`, `subtract`, `minimum`, `sort_order`, `status`, `viewed`, `date_added`, `date_modified`) VALUES (NULL, '$model', '', '0', '0', '0', '0', '0', '0', '$quantity', '7', '$image', '2', '1', '$priceneww', '3', '3', '$today', '0', '0', '0', '0', '0', '0', '1', '1', '0', '1', '0', '$today', '$today');";   
